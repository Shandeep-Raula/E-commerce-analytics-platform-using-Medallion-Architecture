"""
Supabase → Snowflake ETL Pipeline
...
# Map: Supabase source table → Snowflake target table name
...
def discover_tables() -> list[str]:
    """Auto-discover all user tables from Supabase public schema."""
...
def extract_table(table_name: str) -> pd.DataFrame:
    """Extract all rows from a Supabase table via direct PostgreSQL connection."""
    logger = get_run_logger()
    conn = _supabase_conn()
    try:
...
def load_to_snowflake(df: pd.DataFrame, target_table: str, mode: str = "truncate_insert") -> int:
    ...
    try:
        ...
        success, nchunks, nrows, _ = write_pandas(
            ...
            use_logical_type=True,      # fix datetime timezone warning
        )
...
@flow(
    name="supabase-to-snowflake",
    description="Full ETL pipeline: Supabase → Snowflake, orchestrated by Prefect",
    log_prints=True,
)
def etl_pipeline(
    ...
    Args:
        tables:    Explicit list of Supabase tables to sync.
    ...
    for source_table in source_tables:
        ...
        # ✅ Add _loaded_at BEFORE ensure_table_exists so DDL includes the column
        df["_loaded_at"] = datetime.now(timezone.utc)

        # Ensure target table exists in Snowflake (DDL now includes _loaded_at)
...
if __name__ == "__main__":
    # Run locally:
    #   python load_supabse_to_snowflake.py
    #
    # Deploy with a daily schedule:
    #   prefect deploy load_supabse_to_snowflake.py:etl_pipeline \
